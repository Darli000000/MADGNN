...
from Agents.MAD4PG_DynENV.agent import D4PGNetworks, D4PGConfig, D4PGBuilder, get_replicator
...
class DistributedD4PG:
    ...
    def actor(
        ...
        networks = self._networks # NOTICE：这里和learner一样，用的都是D4PGNetworks
        for network in networks: # 一共有agent_number个network
            network.init(self._environment_spec)
        policy_networks = []
        for _ in range(self._agent_number):  # NOTICE: 这里会创建edge_num边缘结点数量个policy_network(该实验中是9个)，来模拟9个决策的agent
        ...
        # Create the agent.
        actor = self._builder.make_actor(  # NOTICE：详见Agents.MAD4PG.agent中的D4PGBuilder
            agent_number=self._agent_number,
            agent_action_size=self._agent_action_size,
            policy_networks=policy_networks,
            adder=self._builder.make_adder(replay),  # NOTICE：详见Agents.MAD4PG.agent中的D4PGBuilder
        ...
        counter = counting.Counter(counter, 'actor') # NOTICE：用于结束训练，数数数到max_actor_steps=50*200
        logger = loggers.make_default_logger(
            'actor',
            save_data=False,
