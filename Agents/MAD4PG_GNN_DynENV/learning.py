...
from Agents.MAD4PG_GNN_DynENV.dynamic_gnn import run_dyn_gnn_batched   # EDIT
...
class D4PGLearner(acme.Learner):
    ...
    def _step_joint_gnn(self, sample) -> Dict[str, tf.Tensor]:
        ...
        snapshot = {
            ...
            "edge_feature": tf.cast(obs["snapshot"]["edge_feature"], tf.float32),
            "edge_node_indices": tf.cast(obs["snapshot"]["edge_node_indices"], tf.int32),
            "edge_mask": tf.cast(obs["snapshot"]["edge_mask"], tf.float32),
        }
        ...
        next_snapshot = {
            ...
            "edge_feature": tf.cast(next_obs["snapshot"]["edge_feature"], tf.float32),
            "edge_node_indices": tf.cast(next_obs["snapshot"]["edge_node_indices"], tf.int32),
            "edge_mask": tf.cast(next_obs["snapshot"]["edge_mask"], tf.float32),
        }
    ...
    def _step_old(self, sample) -> Dict[str, tf.Tensor]:
        ...
        # DEBUG 
        # if self._debug_count < 2:
        #     tf.print("[LEARNER TEST] local_observation shape =", tf.shape(local_observation))
        #     tf.print("[LEARNER TEST] prev_state shape =", tf.shape(prev_state))
        #     tf.print("[LEARNER TEST] snapshot.node_feature shape =", tf.shape(snapshot["node_feature"]))
        #     tf.print("[LEARNER TEST] snapshot.edge_index shape =", tf.shape(snapshot["edge_index"]))
        #     tf.print("[LEARNER TEST] snapshot.edge_feature shape =", tf.shape(snapshot["edge_feature"]))
        #     tf.print("[LEARNER TEST] snapshot.edge_mask shape =", tf.shape(snapshot["edge_mask"]))
        #     tf.print("[LEARNER TEST] edge_mask sum sample0 =", tf.reduce_sum(snapshot["edge_mask"][0]))
        #     self._debug_count += 1
    ...
    def _replicated_step(self):
        ...
        replicated_fetches = self._replicator.run(self._step_joint_gnn, args=(sample,))    # NOTICE: 调用最核心训练函数_step(), 试验一下，改成old版(old版不合并计算梯度)
