...
class FeedForwardActor(core.Actor):
    ...
    def _compute_graph_context(self, snapshot: Dict[str, Any]) -> tf.Tensor:
        ...
        if self._prev_state_list is not None:
            prev_num_nodes = int(self._prev_state_list[0].shape[0])
            curr_num_nodes = int(snapshot["node_feature"].shape[0])
            if prev_num_nodes != curr_num_nodes:
                print(f"[GNN SHAPE DEBUG] prev_num_nodes={prev_num_nodes}, curr_num_nodes={curr_num_nodes}, reset prev_state")
                self._prev_state_list = None
