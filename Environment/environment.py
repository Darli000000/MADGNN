...
class vehicularNetworkEnv(dm_env.Environment):
    ...
    def _observation(self) -> np.ndarray:
        ...
        # print("[DEBUG _observation] vehicle_number_within_edges:", self._config.vehicle_number_within_edges)
        # ================================================================================================= #
    ...
    def _get_graph_connection_range(self) -> float:
        ...
        return float(self._config.communication_range * 1.1)
    ...
    def _build_snapshot_v1(self) -> Dict[str, Any]:
        ...
        for e1, e2 in ee_pairs:
            ...
            feat = np.array([
                _normalize(dist, 0.0, np.sqrt(self._config.map_length**2 + self._config.map_width**2)),
                _normalize(
        ...
        for v_idx in active_vehicle_indices:
            ...
            for edge in edge_list:
                ...
                if dist <= graph_range:
                    ...
                    # 如果 environment 内已有 channel_condition_matrix，优先取它；
                    # 若没有，先用 0 占位
                    channel_gain = 0.0
        ...
        edge_index = np.array([edge_index_src, edge_index_dst], dtype=np.int64)
        edge_feature = np.stack(edge_features, axis=0).astype(np.float32) if len(edge_features) > 0 else np.zeros((0, 4), dtype=np.float32)
        edge_type = np.array(edge_types, dtype=np.int32)
        ...
        snapshot = {
            ...
            "num_edges": int(edge_index.shape[1]) if edge_index.size > 0 else 0,
        }
        return snapshot
