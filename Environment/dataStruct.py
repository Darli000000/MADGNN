...
class vehicleList(object):
    ...
    # DynENV-EDIT : 老的适用于老环境的轨迹读取措施（可以与下面的函数相互替换）
    ...
    #             # 我感觉不用像上面那样写，因为后面本来就会根据区域筛选，直接读取同样的文件就行，换成下面这行
    ...
    def read_vehicle_trajectories(self, timeSlots: timeSlots) -> List[trajectory]:
        ...
        # 只保留当前 timeSlots 范围内的数据
        start_t = timeSlots.get_start()
        end_t = timeSlots.get_end()
        expected_times = list(range(start_t, end_t + 1))
        expected_len = len(expected_times)

        df = df[(df['time'] >= start_t) & (df['time'] <= end_t)].copy()
        ...
        for vehicle_id, g in grouped:
            ...
            vehicle_traj_map[int(vehicle_id)] = trajectory(
                timeSlots=timeSlots,
                locations=loc_list
            )
            candidate_vehicle_ids.append(int(vehicle_id))

        if len(candidate_vehicle_ids) < self._vehicle_number:
            raise ValueError(
                f"Not enough valid vehicles: need {self._vehicle_number}, got {len(candidate_vehicle_ids)}"
            )

        # 方式1：顺序抽取
        selected_vehicle_ids = candidate_vehicle_ids[:self._vehicle_number]

        # 如果你想随机抽取，就改成：
        # rng = np.random.default_rng(0)
        # selected_vehicle_ids = rng.choice(
        #     candidate_vehicle_ids,
        #     size=self._vehicle_number,
        #     replace=False
        # ).tolist()

        vehicle_trajectories = [vehicle_traj_map[vid] for vid in selected_vehicle_ids]
        return vehicle_trajectories
