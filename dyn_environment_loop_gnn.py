...
class EnvironmentLoop(core.Worker):
    ...
    def run_episode(self) -> loggers.LoggingData:
        ...
        # log
        # EDIT: wtf,为什么这里要log，永远是0，我先把他注释了
        #print("episode:", episode_steps)
        ...
        # EDIT: Debug Observation Info
        # =================================================== #
        # print("\n========== DEBUG RESET OBS ==========")
        # obs_np = np.array(timestep.observation)
        # print("reset obs shape:", obs_np.shape)
        # print(obs_np)
        # for agent_i in range(obs_np.shape[0]):
        #     print(f"[reset] agent {agent_i} obs shape: {obs_np[agent_i].shape}")
        #     print(obs_np[agent_i])
        # print("=====================================\n")
        # =================================================== #
        ...
        # == EDIT: edited old _actor.observe_first() to fit the new relay spec == #
        # self._actor.observe_first(timestep)
        first_snapshot = self._environment.get_current_snapshot()
        first_prev_state = self._actor.get_prev_state_for_replay(first_snapshot)

        first_snapshot = _snapshot_for_replay(first_snapshot)

        wrapped_first_timestep = timestep._replace(
            observation={
                "local_observation": timestep.observation,
                "snapshot": first_snapshot,
                "prev_state": first_prev_state,
            }
        )

        self._actor.observe_first(wrapped_first_timestep)
        # == EDIT: edited old _actor.observe_first() to fit the new relay spec == #
        ...
        while not timestep.last():
            # EDIT_ADD：在这里注释。optional，先去掉，加上之后log太多
        ...
        # Generate an action from the agent's policy and step the environment.
            ...
            # ==================== DymGNN Version ==================== #
            # actor 的输入直接用未清洗的snapshot即可
            current_snapshot = self._environment.get_current_snapshot()
            actor_input = {
                "local_observation": timestep.observation,
                "snapshot": current_snapshot,
            }

            action = self._actor.select_action(actor_input)
            # ==================== DymGNN Version ==================== #

            # if episode_steps < 3:
            #     print("snapshot time:", current_snapshot["time_index"])
            #     print("local obs shape:", np.array(timestep.observation).shape)
            #     print("action shape:", np.array(action).shape)
            #     print("prev_state_shapes:", self._actor.get_prev_state_shapes())

            # =========== EDIT: snapshot debug =========== #
            # if episode_steps < 3:
            #     snap = self._environment.get_current_snapshot()
            #     self._environment.debug_print_snapshot(snap)
            # =========== EDIT: snapshot debug =========== #
            ...
            # ====== EDIT: edited old observe() into gnn_live_update version ====== #
            # after select_action, actor internal prev_state has already become H_t
            # 使用清洗后的snapshot，并保存prev_state
            next_snapshot = self._environment.get_current_snapshot()
            next_prev_state = self._actor.get_prev_state_for_replay(next_snapshot)
            next_snapshot = _snapshot_for_replay(next_snapshot)

            wrapped_next_timestep = timestep._replace(
                observation={
                    "local_observation": timestep.observation,
            ...
            self._actor.observe(action, next_timestep=wrapped_next_timestep)
            # ====== EDIT: edited old observe() into gnn_live_update version ====== #
            
            # DEBUG
            # if episode_steps < 2 or episode_steps > 50 and episode_steps < 53:
            #     print(f"\n========== DEBUG NEXT TIMESTEP BUNDLE step={episode_steps} ==========")
            #     obs = wrapped_next_timestep.observation
            #     print("keys:", obs.keys())
            #     print("local_observation shape:", np.array(obs["local_observation"]).shape)
            #     print("prev_state shape:", np.array(obs["prev_state"]).shape)

            #     print("snapshot time_index:", obs["snapshot"]["time_index"])
            #     print("snapshot node_feature shape:", np.array(obs["snapshot"]["node_feature"]).shape)
            #     print("snapshot edge_index shape:", np.array(obs["snapshot"]["edge_index"]).shape)
            #     print("snapshot edge_feature shape:", np.array(obs["snapshot"]["edge_feature"]).shape)
            #     print("===============================================================\n")

            #     print(f"\n========== DEBUG REPLAY SNAPSHOT step={episode_steps} ==========")
            #     print("snapshot keys:", obs["snapshot"].keys())
            #     for k, v in obs["snapshot"].items():
            #         arr = np.asarray(v)
            #         print(k, "shape=", arr.shape, "dtype=", arr.dtype)
            #         if k == "edge_mask":
            #             print("mask detail=",arr)
            #     print("==========================================\n")
        ...
        # EDIT OPTIONAL: DEBUG log counts
...
# cleaning function: clean the original snapshot data
def _snapshot_for_replay(snapshot):
    ...
    replay_snapshot = {
        "node_feature": np.asarray(snapshot["node_feature"], dtype=np.float32),
        "node_type": np.asarray(snapshot["node_type"], dtype=np.int32),
        "edge_index": np.asarray(snapshot["edge_index"], dtype=np.int32),
        "edge_feature": np.asarray(snapshot["edge_feature"], dtype=np.float32),
        "edge_type": np.asarray(snapshot["edge_type"], dtype=np.int32),
        "edge_mask": np.asarray(snapshot["edge_mask"], dtype=np.float32),   # DynENV-EDIT
        "edge_node_indices": edge_node_indices,
        "time_index": np.asarray(snapshot["time_index"], dtype=np.int32),
    }
